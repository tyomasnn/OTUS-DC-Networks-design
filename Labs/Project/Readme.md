...
- Кластер серверов - обработчиков поступающего трафика с функционалом:
  * Формирования статистической информации для поступающего на обработку трафика - индексы (текстовые журналы, файлы) и их отправки на Кластер серверов накопления и хранения статистической информации и содержимого;
  * Обработки поступающей информации и отправки потока обработанного содержимого на Кластер серверов накопления и хранения статистической информации и содержимого.
...
- Первый IP-адрес из подсети 192.168.52.0/24 в ЦОД-52 либо подсети 192.168.63.0/24 в ЦОД-63 находятся в изолировнном vrf "vrf-MGMT" - используется для удаленного доступа Технической поддержки ПАК, со стороны рабочего места которой используется подсеть 192.168.255.0/24 и является маршрутизируемым (L3 VNI);
- Второй IP-адрес из подсети 192.168.152.0/24 в ЦОД-52 либо подсети 192.168.163.0/24 в ЦОД-63 находятся в изолировнном vrf "vrf-DATA" - используется для передачи статистической информации от Кластера серверов - обработчиков на Кластер серверов хранения статистической информации и содержимого, а так же выгрузки накопленной статистической информации и содержимого по запросу Пользователя ПАК со стороны рабочего места которого используется подсеть 192.168.254.0/24 и является маршрутизируемым (L3 VNI);
- Третий IP-адрес из подсети 192.168.74.0/24 в ЦОД-52 и в ЦОД-63 находятся в изолировнном vrf "vrf-MGMT" - используется для передачи потока содержимого от Кластера серверов - обработчиков на Кластер серверов хранения статистической информации и содержимого и является НЕмаршрутизируемым (L2 VNI).

vrf "vrf-MGMT" и vrf "vrf-DATA" между собой изолированы, меж-VRF связность внутри VXLAN-фабрики - запрещена.

Для обеспечения подключения к VXLAN фабрике и доступа к элементам ПАК внешних пользователей, а именно:
- рабочего места Технической поддержки ПАК, для которого выделена отдельная подсеть 192.168.255.0/24 находящейся в изолировнном vrf "vrf-MGMT";
- рабочего места Пользователя ПАК, для которого выделена отдельная подсеть 192.168.254.0/24 находящейся в изолировнном vrf "vrf-DATA"
Используется отдельный L2 коммутатор доступа, расположенный в ЦОД 52, от которого, с целью обеспечения резервирования собраны два линка к BGW1-52 и BGW2-52, объединенных в ESI LAG. В свою очередь линки между коммутатором доступа и BGW объединены в LAG с включенным протоколом LACP.

Рабочие места Технической поддержки и Пользователя ПАК располагаются в ЦОД 52.
...
В среде виртуализации EVE-NG cобрана и настроена топология Underlay/Overlay сети Spine-Leaf для двух ЦОД - ЦОД-52 и ЦОД-63, которые через DCI interconnect и два RouteServer-а объединены в топологию Multisite.<br>
МежЦОДовые каналы могут быть реализованы на базе:
* Темных волокон;
* DWDM каналов;
* X-Connect с обеспечением на канале MTU не менее 9000.

При расширении ПАК и выносе сегментов в новые ЦОДы топология легко может масштабироваться при подключении сегментов расширения ПАК к действующим RouteServer-ам и их добавлением в действующий Multisite.

В качестве всех элементов лабораторной сборки, включая конечные устройства (Server) и коммутатор доступа - используются L3-коммутаторы Arista.<br>
